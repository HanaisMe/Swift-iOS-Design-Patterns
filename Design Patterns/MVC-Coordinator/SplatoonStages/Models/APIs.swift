...
// MARK: - https://stat.ink/start
// `stat.ink provides an API to the public. You can design your own software if you want.`
enum StatInkAPI {
    static let domain = "https://stat.ink/api/v2"
    
    enum StatInkApiType {
        case weaponList
        case weaponDetail(key: String)
    }
    
    static func getUrlString(type: StatInkApiType) -> String {
        switch type {
        case .weaponList:
            return domain + "/weapon"
        case .weaponDetail(let key):
            return domain + "/search?key=\(key)"
        }
    }
}
